# dir2site figures — VS Code Markdown preview
...
dir2site ones, so it can be turned off with `dir2siteFigures.hardLineBreaks`.
...
Both rules are written out longhand in `markdown-it-dir2site-figures.js` rather than pulled from
npm, so there is nothing to install or bundle.

## Rebuilding the packaged copy

The app ships `Assets/editors/dir2site-figures.vsix` and installs it from a button. That file is
...
code --install-extension dir2site-figures-0.1.0.vsix
```
